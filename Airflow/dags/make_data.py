...
default_args = {
    ...
    'start_date': datetime(2023,7,26,14),
...
trigger_path = os.path.join(airflow_path,'trigger','retrain.txt')

with DAG(
    dag_id = 'make_data_dag',         # dag 
    default_args = default_args,    # dag 기본 옵션
    schedule_interval= "0 14 * * *", # 매 23시마다 실행하겠다 UTC 기준 14시
    tags=['make_data_dags'],
    catchup=False,
) as dag:
    
    # github airflow의 make_data 코드
    bash_command = f"python {os.path.join(airflow_path,'func','data.py')}"
